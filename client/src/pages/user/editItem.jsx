...
function EditItem() {
  ...
  const { id } = useParams(); // To get the item ID from URL if needed

  // Mock fetch data effect
  useEffect(() => {
    // You would fetch existing item data here based on the ID
    // setEditItemData(fetchedData);
    // if (fetchedData.image) setPreview(fetchedData.imageUrl);
  }, [id]);
                  ...
                  {preview ? (
                    <div className="flex flex-col items-center justify-center h-full w-full bg-white">
                      <div className="flex relative flex-col p-3 pb-2 justify-between items-center h-56 w-56 border border-slate-300 rounded-xl bg-white shadow-sm">
                        ...
                        <div className="flex items-center justify-between w-full pt-1">
                          <FieldLabel htmlFor="item-image2" className={`text-xs font-bold ${isLoading ? "text-slate-400" : "text-orange-500 hover:text-orange-600"} ml-1 cursor-pointer uppercase tracking-wider`}>
                            Change Image
                            <Input disabled={isLoading} id="item-image2" type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
                          </FieldLabel>
                          <Button
                            disabled={isLoading}
                            variant="ghost"
                    ...
                    <div className="flex justify-center items-center w-full h-full bg-white">
                      <div className="w-56 h-56 flex flex-col items-center justify-center border border-slate-300 border-dashed rounded-xl cursor-pointer hover:border-orange-500 transition-colors bg-white shadow-sm p-4 text-center">
                        <Input disabled={isLoading} id="item-image" type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
